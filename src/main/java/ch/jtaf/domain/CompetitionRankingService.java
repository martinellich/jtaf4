...
public class CompetitionRankingService {
    ...
    public byte[] getDiplomasAsPdf(Long competitionId, Locale locale) {
        return new DiplomaReport(getCompetitionRanking(competitionId).orElseThrow(), getLogo(competitionId), locale)
            .create();
    }
    ...
    public Optional<CompetitionRankingData> getCompetitionRanking(Long competitionId) {
        return dslContext.select(COMPETITION.NAME, COMPETITION.COMPETITION_DATE, COMPETITION.ALWAYS_FIRST_THREE_MEDALS,
                COMPETITION.MEDAL_PERCENTAGE,
                multiset(select(CATEGORY.ABBREVIATION, CATEGORY.NAME, CATEGORY.YEAR_FROM, CATEGORY.YEAR_TO,
                        multiset(select(CATEGORY_ATHLETE.athlete().FIRST_NAME, CATEGORY_ATHLETE.athlete().LAST_NAME,
                                ...
                                multiset(select(RESULT.event().ABBREVIATION, RESULT.RESULT_, RESULT.POINTS).from(RESULT)
                                    ...
                                    .orderBy(RESULT.POSITION)).convertFrom(
                                            r -> r.map(mapping(CompetitionRankingData.Category.Athlete.Result::new))))
    ...
    public Optional<EventsRankingData> getEventsRanking(Long competitionId) {
        return dslContext
            .select(COMPETITION.NAME, COMPETITION.COMPETITION_DATE,
                    multiset(select(EVENT.ABBREVIATION, EVENT.GENDER, EVENT.EVENT_TYPE,
                            multiset(selectDistinct(RESULT.athlete().LAST_NAME, RESULT.athlete().FIRST_NAME,
                                    RESULT.athlete().YEAR_OF_BIRTH, RESULT.category().ABBREVIATION,
                                    RESULT.athlete().club().NAME, RESULT.RESULT_)
                                .from(RESULT)
    ...
    private byte[] getLogo(Long competitionId) {
        var logoRecord = dslContext.select(COMPETITION.series().LOGO)
            .from(COMPETITION)
            .where(COMPETITION.ID.eq(competitionId))
            .fetchOne();
        if (logoRecord != null) {
