...
public class AthleteDAO extends JooqDAO<Athlete, AthleteRecord, Long> {
    ...
    public List<AthleteRecord> findBySeriesId(Long seriesId) {
        return dslContext.select(CATEGORY_ATHLETE.athlete().fields())
            .from(CATEGORY_ATHLETE)
            .where(CATEGORY_ATHLETE.category().SERIES_ID.eq(seriesId))
            .orderBy(CATEGORY_ATHLETE.category().ABBREVIATION, CATEGORY_ATHLETE.athlete().LAST_NAME,
                    CATEGORY_ATHLETE.athlete().FIRST_NAME)
            .fetchInto(ATHLETE);
    }

    public List<AthleteRecord> findByOrganizationIdAndSeriesId(long organizationId, long seriesId, Condition condition,
            int offset, int limit) {
        return dslContext.selectFrom(ATHLETE)
            .where(ATHLETE.ORGANIZATION_ID.eq(organizationId))
            .and(ATHLETE.ID.notIn(select(CATEGORY_ATHLETE.ATHLETE_ID).from(CATEGORY_ATHLETE)
                .join(CATEGORY)
    ...
    public int countByOrganizationIdAndSeriesId(Long organizationId, Long seriesId, Condition condition) {
        return dslContext.selectCount()
            .from(ATHLETE)
            .where(ATHLETE.ORGANIZATION_ID.eq(organizationId))
            .and(ATHLETE.ID.notIn(dslContext.select(CATEGORY_ATHLETE.ATHLETE_ID)
                .from(CATEGORY_ATHLETE)
                .join(CATEGORY)
                .on(CATEGORY.ID.eq(CATEGORY_ATHLETE.CATEGORY_ID))
                .where(CATEGORY.SERIES_ID.eq(seriesId))))
    ...
    public Result<Record4<Long, String, String, Long>> getAthletes(long competitionId, Condition condition, int offset,
            int limit) {
        return dslContext.select(ATHLETE.ID, ATHLETE.LAST_NAME, ATHLETE.FIRST_NAME, CATEGORY.ID)
            .from(ATHLETE)
            .join(CATEGORY_ATHLETE)
            .on(CATEGORY_ATHLETE.ATHLETE_ID.eq(ATHLETE.ID))
            .join(CATEGORY)
            .on(CATEGORY.ID.eq(CATEGORY_ATHLETE.CATEGORY_ID))
            .join(COMPETITION)
            .on(COMPETITION.SERIES_ID.eq(CATEGORY.SERIES_ID))
