...
class UC025CopyCategoriesTest extends AbstractViewTest {
	...
	void add_series_and_copy_categories() {
		...
		// Copy series
		$(Button.class).id("copy-categories").click();

		ComboBox<SeriesRecord> seriesSelection = $(ComboBox.class).id("series-selection");
		seriesSelection.setValue(seriesSelection.getLazyDataView().getItem(0));
		$(Button.class).id("copy-categories-copy").click();

		Notification copiedNotification = $(Notification.class).single();
		...
		// Remove series
		navigate(SeriesListView.class);
		Grid<SeriesRecord> refreshedGrid = $(Grid.class).id("series-grid");
